...
describe("NextAppPathStrategy", () => {
  ...
  describe("default type parameters", () => {
    it("PA defaults to defaultConfig PathAtlas instance", () => {
      expectTypeOf<NextAppPathStrategy<TestAtlas>["config"]["PathAtlas"]>().toEqualTypeOf<PathAtlasCtor<DefaultPA>>();
    });

    it("LK defaults to defaultConfig localeKey", () => {
      expectTypeOf<NextAppPathStrategy<TestAtlas>["config"]["localeKey"]>().toEqualTypeOf<DefaultLK>();
    });
  });
  ...
  describe("public properties", () => {
    it("rMachine is RMachine<RA>", () => {
      expectTypeOf<NextAppPathStrategy<TestAtlas>["rMachine"]>().toEqualTypeOf<RMachine<TestAtlas>>();
    });

    it("config.cookie is CookieOption", () => {
      type CookieOption = "on" | "off" | CookieDeclaration;
      expectTypeOf<NextAppPathStrategy<TestAtlas>["config"]["cookie"]>().toEqualTypeOf<CookieOption>();
    });
    ...
    it("config.autoLocaleBinding is SwitchableOption", () => {
      expectTypeOf<NextAppPathStrategy<TestAtlas>["config"]["autoLocaleBinding"]>().toEqualTypeOf<"on" | "off">();
    });

    it("config.basePath is string", () => {
      expectTypeOf<NextAppPathStrategy<TestAtlas>["config"]["basePath"]>().toEqualTypeOf<string>();
    });
    ...
    it("config.autoDetectLocale includes custom object form", () => {
      ...
      expectTypeOf<{ readonly pathMatcher: null }>().toExtend<
        NextAppPathStrategy<TestAtlas>["config"]["autoDetectLocale"]
      >();
      expectTypeOf<CustomAutoDetectLocale>().toExtend<NextAppPathStrategy<TestAtlas>["config"]["autoDetectLocale"]>();
    });

    it("config.autoDetectLocale accepts 'on' and 'off' string values", () => {
      expectTypeOf<"on">().toExtend<NextAppPathStrategy<TestAtlas>["config"]["autoDetectLocale"]>();
      expectTypeOf<"off">().toExtend<NextAppPathStrategy<TestAtlas>["config"]["autoDetectLocale"]>();
    });
    ...
    it("config.implicitDefaultLocale includes custom object form", () => {
      ...
      expectTypeOf<CustomImplicitDefaultLocale>().toExtend<
        NextAppPathStrategy<TestAtlas>["config"]["implicitDefaultLocale"]
      >();
    });

    it("config.implicitDefaultLocale accepts 'on' and 'off' string values", () => {
      expectTypeOf<"on">().toExtend<NextAppPathStrategy<TestAtlas>["config"]["implicitDefaultLocale"]>();
      expectTypeOf<"off">().toExtend<NextAppPathStrategy<TestAtlas>["config"]["implicitDefaultLocale"]>();
    });
  });
  ...
  describe("toolset return types", () => {
    it("createClientToolset returns Promise<NextAppClientToolset<RA, DefaultPA>>", () => {
      expectTypeOf<NextAppPathStrategy<TestAtlas>["createClientToolset"]>().returns.toEqualTypeOf<
        Promise<NextAppClientToolset<TestAtlas, DefaultPA>>
      >();
    });

    it("createClientToolset accepts no parameters", () => {
      expectTypeOf<NextAppPathStrategy<TestAtlas>["createClientToolset"]>().parameters.toEqualTypeOf<[]>();
    });

    it("createServerToolset accepts NextAppClientRMachine", () => {
      expectTypeOf<NextAppPathStrategy<TestAtlas>["createServerToolset"]>()
        .parameter(0)
        .toEqualTypeOf<NextAppClientRMachine>();
    });

    it("createServerToolset returns Promise<NextAppServerToolset<RA, DefaultPA, DefaultLK>>", () => {
      expectTypeOf<NextAppPathStrategy<TestAtlas>["createServerToolset"]>().returns.toEqualTypeOf<
        Promise<NextAppServerToolset<TestAtlas, DefaultPA, DefaultLK>>
      >();
    });

    it("createNoProxyServerToolset accepts NextAppClientRMachine", () => {
      expectTypeOf<NextAppPathStrategy<TestAtlas>["createNoProxyServerToolset"]>()
        .parameter(0)
        .toEqualTypeOf<NextAppClientRMachine>();
    });

    it("createNoProxyServerToolset returns Promise<NextAppNoProxyServerToolset<RA, DefaultPA, DefaultLK>>", () => {
      expectTypeOf<NextAppPathStrategy<TestAtlas>["createNoProxyServerToolset"]>().returns.toEqualTypeOf<
        Promise<NextAppNoProxyServerToolset<TestAtlas, DefaultPA, DefaultLK>>
      >();
    });
  });
  ...
  describe("custom type parameters", () => {
    it("custom PA is wired through to config.PathAtlas", () => {
      expectTypeOf<NextAppPathStrategy<TestAtlas, TranslatedPathAtlas>["config"]["PathAtlas"]>().toEqualTypeOf<
    ...
    it("custom LK is reflected in config.localeKey", () => {
      expectTypeOf<
        NextAppPathStrategy<TestAtlas, SimplePathAtlas, "lang">["config"]["localeKey"]
      >().toEqualTypeOf<"lang">();
    });
    ...
    it("custom LK affects server toolset return type", () => {
      expectTypeOf<
        NextAppPathStrategy<TestAtlas, SimplePathAtlas, "lang">["createServerToolset"]
    ...
    it("custom PA+LK affects no-proxy server toolset return type", () => {
      expectTypeOf<
        NextAppPathStrategy<TestAtlas, TranslatedPathAtlas, "lang">["createNoProxyServerToolset"]
      >().returns.toEqualTypeOf<Promise<NextAppNoProxyServerToolset<TestAtlas, TranslatedPathAtlas, "lang">>>();
    });

    it("different RA produce different types", () => {
      type OtherAtlas = { readonly other: { readonly value: number } };
      expectTypeOf<NextAppPathStrategy<TestAtlas>>().not.toEqualTypeOf<NextAppPathStrategy<OtherAtlas>>();
    });

    it("different PA produce different types", () => {
      expectTypeOf<NextAppPathStrategy<TestAtlas, SimplePathAtlas>>().not.toEqualTypeOf<
        NextAppPathStrategy<TestAtlas, TranslatedPathAtlas>
      >();
    });

    it("different LK produce different types", () => {
      expectTypeOf<NextAppPathStrategy<TestAtlas, SimplePathAtlas, "locale">>().not.toEqualTypeOf<
        NextAppPathStrategy<TestAtlas, SimplePathAtlas, "lang">
      >();
    });
  });
  ...
  describe("hrefHelper", () => {
    it("getPath is a function that rejects non-string locale", () => {
      type GetPath = NextAppPathStrategy<TestAtlas, TranslatedPathAtlas>["hrefHelper"]["getPath"];
      expectTypeOf<GetPath>().toBeFunction();

      const strategy = null! as NextAppPathStrategy<TestAtlas, TranslatedPathAtlas>;
    ...
    it("does not have getUrl property (unlike origin strategy)", () => {
      type Helper = NextAppPathStrategy<TestAtlas, TranslatedPathAtlas>["hrefHelper"];
