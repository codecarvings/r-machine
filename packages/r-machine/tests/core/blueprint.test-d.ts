...
import type { AnyResOrigin, ResOriginType } from "../../src/core/res.js";
...
describe("Blueprint", () => {
  ...
  it("types `layoutEntryType` as the full ResLayoutEntryType union", () => {
    // The blueprint preserves the original layout literal so callers can
    // recover the path-level specialisation (e.g. shell vs shell(mono),
    // gear:outer vs gear:outer(vertex)).
    expectTypeOf<Blueprint["layoutEntryType"]>().toEqualTypeOf<ResLayoutEntryType>();
  });

  it("types `family` as ResFamily and intentionally excludes layout-level literals", () => {
    // The blueprint is post-resolution: layout types like "shell(mono)" or
    // "gear:outer(vertex)" collapse to the family "shell" / "gear" at build
    // time, so the family union never sees them.
    ...
    expectTypeOf<"gear:inner">().not.toExtend<Blueprint["family"]>();
  });

  it("types `gearRole` as GearRole | undefined (raw resources and shells have no role)", () => {
  ...
  it("types `originType` as the closed union of the two canonical origin kinds", () => {
    expectTypeOf<Blueprint["originType"]>().toEqualTypeOf<ResOriginType>();
    expectTypeOf<Blueprint["originType"]>().toEqualTypeOf<"res-matrix" | "res">();
    expectTypeOf<string>().not.toExtend<Blueprint["originType"]>();
  });

  it("types `origin` as the full AnyResOrigin union (matrix | raw resource)", () => {
    expectTypeOf<Blueprint["origin"]>().toEqualTypeOf<AnyResOrigin>();
  });
});

describe("createBlueprint — signature", () => {
  it("takes (AnyResModule, AnyNamespace, AnyLocale | undefined, ResLayoutEntryType) in this exact order", () => {
  ...
  it("does not refine the return type based on the layout literal passed in", () => {
    ...
    const d1 = createBlueprint(module, "ns", undefined, "gear:inner");
  ...
  it("requires the locale argument to be passed explicitly (undefined is a value, not an omission)", () => {
    ...
    createBlueprint(module, "ns", "gear:inner");
    // Baseline: explicit undefined is accepted.
    createBlueprint(module, "ns", undefined, "gear:inner");
  });

  it("rejects a module that does not satisfy AnyResModule", () => {
    // @ts-expect-error — missing `r`
    createBlueprint({}, "ns", undefined, "gear:inner");
    // @ts-expect-error — `r` must be a resource origin, not a primitive
    createBlueprint({ r: "nope" }, "ns", undefined, "gear:inner");
    // @ts-expect-error — null is not an AnyResModule
    createBlueprint(null, "ns", undefined, "gear:inner");
  });

  it("rejects a namespace that is not a string", () => {
    const module: AnyResModule = { r: {} };
    // @ts-expect-error — number is not AnyNamespace
    createBlueprint(module, 42, undefined, "gear:inner");
    // @ts-expect-error — symbol is not AnyNamespace
    createBlueprint(module, Symbol("ns"), undefined, "gear:inner");
  });

  it("rejects a locale that is neither string nor undefined", () => {
    const module: AnyResModule = { r: {} };
    // @ts-expect-error — number is not a valid locale
    createBlueprint(module, "ns", 0, "gear:inner");
    // @ts-expect-error — null is explicitly excluded (only undefined is allowed for "no locale")
    createBlueprint(module, "ns", null, "gear:inner");
  });

  it("rejects a layoutEntryType that is not a canonical layout literal", () => {
    const module: AnyResModule = { r: {} };
    // @ts-expect-error — "custom" is not a ResLayoutEntryType
    createBlueprint(module, "ns", undefined, "custom");
    // @ts-expect-error — capitalized variant is not a ResLayoutEntryType
    createBlueprint(module, "ns", undefined, "Gear:inner");
    // @ts-expect-error — old "gear" literal (before the role split) is no longer valid
  ...
  it("accepts every canonical layout literal at the call site", () => {
    const module: AnyResModule = { r: {} };
    createBlueprint(module, "ns", undefined, "gear:inner");
    createBlueprint(module, "ns", undefined, "gear:base");
    createBlueprint(module, "ns", undefined, "gear:outer");
    createBlueprint(module, "ns", undefined, "gear:outer(vertex)");
