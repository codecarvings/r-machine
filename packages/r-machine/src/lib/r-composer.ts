...
export interface RComposer<RA extends AnyResourceAtlas, L extends AnyLocale, KA extends NamespaceMap<RA>> {
  define<T>(factory: ($: RCtx<L, RMap<RA, KA>>) => T): T;
  withType<T>(): {
    define<T2 extends T | Promise<T>>(factory: ($: RCtx<L, RMap<RA, KA>>) => T2): T2;
  };
}
