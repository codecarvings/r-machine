...
// `res.perLocale(...)`. The shell resolver's namespace is typed as the CHEAP full-shape union
...
// builder (`ShellResolverBuilder<RA["shape@shell"]>`); the catalog only needs to ADMIT
// a shell resolver, and the specific `ShellResolverHandle<"shell/x">` is preserved via const
...
export type DepHandleMap<RA extends AnyResAtlas, C extends ResAtlasCatalog = "shape"> = {
  readonly [k: string]: Handle<RA[C]> | ShellResolverHandle<Namespace<RA["shape"]>>;
};
...
// Dep-facing variant of SurfaceMap: identical, except a `ShellResolverHandle<S>` (a
...
export type DepSurfaceMap<RA extends AnyResAtlas, HM extends HandleMap<RA>> = {
  -readonly [K in keyof HM]: HM[K] extends ShellResolverHandle<infer S extends string, infer L>
