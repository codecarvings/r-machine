      ...
      <CardHeader className="relative space-y-4 pb-4">
        <div className="flex items-start justify-between">
          ...
          <div className="size-10 rounded-full bg-linear-to-br from-stone-500/20 to-stone-500/20 group-hover:from-red-500/20 group-hover:to-red-500/20 flex items-center justify-center ring-1 ring-stone-500/20 group-hover:ring-red-500/40 transition-all duration-300">
            {icon || <Check className="size-5 text-stone-600 group-hover:text-red-600" />}
          </div>
        </div>
        <CardTitle className="text-xl font-semibold leading-tight">{title}</CardTitle>
      </CardHeader>
      <CardContent className="relative">
        <CardDescription className="text-base leading-relaxed text-muted-foreground/90">{children}</CardDescription>
      </CardContent>
    </Card>
  );
}
