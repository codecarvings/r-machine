    ...
    <section className="container mx-auto px-4 sm:px-6 lg:px-8 pb-16 sm:pb-24">
      <div className="max-w-6xl mx-auto">
        <div className="text-center space-y-4 mb-16">
          <h2 className="text-3xl sm:text-4xl font-bold tracking-tight">{demo.sectionTitle}</h2>
          <p className="text-lg text-muted-foreground">{demo.sectionSubtitle}</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {/* Date formatting */}
          <FeatureBox
            badge={demo.dateTime.badge}
            title={demo.dateTime.label}
          ...
          >
            ...
            <div className="text-xs text-muted-foreground">{now ? demo.dateTime.caption(now) : "---"}</div>
          </FeatureBox>

          {/* Number & Currency formatting */}
          <FeatureBox
            badge={demo.number.badge}
            title={demo.number.label}
            icon={<Hash className="size-5 text-stone-600 group-hover:text-red-600" />}
          >
            <div className="text-lg leading-relaxed text-foreground">{demo.number.description(1234.56)}</div>
          </FeatureBox>

          {/* Plural rules */}
          <FeatureBox
            badge={demo.plural.badge}
            title={demo.plural.label}
            icon={<Users className="size-5 text-stone-600 group-hover:text-red-600" />}
          >
            <div className="text-lg leading-relaxed text-foreground">
              <demo.plural.Items count={count} />
            </div>
