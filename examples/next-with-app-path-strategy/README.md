...
| **OuterGear (the one gear)** | [`outer/timer`](src/r-machine/outer/timer.ts) | The interval and its `Symbol.dispose` cleanup live in the gear, not the component. Tick state survives locale navigation. |
